...
// the mailbox at all. drive.readonly lets the portal read the application sheet
// and the report cards uploaded through the form without either being shared
// publicly; it also covers the Sheets API, so no separate spreadsheets scope is
// needed. openid+email are only there so the callback can show which account
// got connected.
export const SCOPES = [
...
// "ANHS CSF <addr>" rather than a bare address, so the inbox shows the chapter
// name. The name is quoted because RFC 5322 specials like "." or "," would
// otherwise break the address; non-ASCII names use an encoded-word instead.
function mailbox(name, address) {
  const addr = oneLine(address);
  const display = oneLine(name).replace(/["\\]/g, '');
  if (!display || !addr) return addr;
  const shown = /^[\x20-\x7E]*$/.test(display) ? `"${display}"` : header(display);
  return `${shown} <${addr}>`;
}

export function buildMime({ from, fromName, to, subject, body }) {
  const lines = [
